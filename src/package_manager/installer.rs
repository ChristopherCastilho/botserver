    ...
    fn register_drive(&mut self) {
        ...
        self.components.insert(
            ...
            ComponentConfig {
                ...
                download_url: Some(
                    "https://dl.min.io/server/minio/release/windows-amd64/minio.exe".to_string(),
                ),
                binary_name: Some("minio.exe".to_string()),
    ...
    fn register_tables(&mut self) {
        ...
        self.components.insert(
            ...
            ComponentConfig {
                ...
                windows_packages: vec![],
                download_url: Some(
                    "https://get.enterprisedb.com/postgresql/postgresql-18.0-1-windows-x64-binaries.zip".to_string(),
                ),
                binary_name: Some("postgres.exe".to_string()),
                pre_install_cmds_linux: vec![],
                post_install_cmds_linux: vec![
                ...
                pre_install_cmds_windows: vec![],
                post_install_cmds_windows: vec![
                    "if not exist \"{{DATA_PATH}}\\pgdata\" mkdir \"{{DATA_PATH}}\\pgdata\"".to_string(),
                    "initdb -D \"{{DATA_PATH}}\\pgdata\" -U gbuser --pwfile=<(echo %PG_PASSWORD%)".to_string(),
                    "echo data_directory = '{{DATA_PATH}}\\pgdata' > {{CONF_PATH}}\\postgresql.conf".to_string(),
                    "echo ident_file = '{{CONF_PATH}}\\pg_ident.conf' >> {{CONF_PATH}}\\postgresql.conf".to_string(),
                    "echo port = 5432 >> {{CONF_PATH}}\\postgresql.conf".to_string(),
                    "echo listen_addresses = '*' >> {{CONF_PATH}}\\postgresql.conf".to_string(),
                    ...
                    "pg_ctl -D \"{{DATA_PATH}}\\pgdata\" -l \"{{LOGS_PATH}}\\postgres.log\" start -w -t 30".to_string(),
                    "timeout 5".to_string(),
                    "for /l %i in (1,1,30) do (pg_isready -h localhost -p 5432 -U gbuser >nul 2>&1 && echo PostgreSQL is ready && goto :ready || echo Waiting for PostgreSQL... attempt %i/30 >&2 & timeout 2 >nul)".to_string(),
                    ":ready".to_string(),
                    "pg_isready -h localhost -p 5432 -U gbuser || (echo ERROR: PostgreSQL failed to start properly >&2 & type \"{{LOGS_PATH}}\\postgres.log\" >&2 & exit /b 1)".to_string(),
                    "psql -h localhost -p 5432 -U gbuser -d postgres -c \"CREATE DATABASE botserver WITH OWNER gbuser\" 2>&1 | find /v \"already exists\" || ver >nul".to_string(),
                ],
    ...
    fn register_cache(&mut self) {
        self.components.insert(
            ...
            ComponentConfig {
                ...
                windows_packages: vec![],
                download_url: Some(
                    "https://github.com/redis-windows/redis-windows/releases/download/7.0.12/Redis-x64-7.0.12.zip".to_string(),
                ),
                binary_name: Some("redis-server.exe".to_string()),
                pre_install_cmds_linux: vec![],
                post_install_cmds_linux: vec![
                ...
                pre_install_cmds_windows: vec![],
                post_install_cmds_windows: vec![
                    "{{BIN_PATH}}\\redis-server.exe --service-install {{CONF_PATH}}\\redis.conf".to_string(),
                    "{{BIN_PATH}}\\redis-server.exe --service-start".to_string(),
                ],
                env_vars: HashMap::new(),
                data_download_list: Vec::new(),
    ...
    fn register_llm(&mut self) {
        self.components.insert(
            ...
            ComponentConfig {
                ...
                windows_packages: vec!["7zip".to_string()],
                download_url: Some(
                    "https://github.com/ggerganov/llama.cpp/releases/download/b6148/llama-b6148-bin-win-x64.zip".to_string(),
                ),
                binary_name: Some("llama-server.exe".to_string()),
                pre_install_cmds_linux: vec![],
                post_install_cmds_linux: vec![],
                pre_install_cmds_macos: vec![],
                post_install_cmds_macos: vec![],
    ...
    fn register_email(&mut self) {
        self.components.insert(
            ...
            ComponentConfig {
                ...
                windows_packages: vec![],
                download_url: Some(
                    "https://github.com/stalwartlabs/stalwart/releases/download/v0.13.1/stalwart-x86_64-pc-windows-msvc.zip".to_string(),
                ),
                binary_name: Some("stalwart.exe".to_string()),
                pre_install_cmds_linux: vec![],
                post_install_cmds_linux: vec![
    ...
    fn register_proxy(&mut self) {
        self.components.insert(
            ...
            ComponentConfig {
                ...
                windows_packages: vec![],
                download_url: Some(
                    "https://github.com/caddyserver/caddy/releases/download/v2.10.0-beta.3/caddy_2.10.0-beta.3_windows_amd64.zip".to_string(),
                ),
                binary_name: Some("caddy.exe".to_string()),
                pre_install_cmds_linux: vec![],
                post_install_cmds_linux: vec![
    ...
    fn register_directory(&mut self) {
        self.components.insert(
            ...
            ComponentConfig {
                ...
                windows_packages: vec![],
                download_url: Some(
                    "https://github.com/zitadel/zitadel/releases/download/v2.71.2/zitadel-windows-amd64.zip".to_string(),
                ),
                binary_name: Some("zitadel.exe".to_string()),
                pre_install_cmds_linux: vec![],
                post_install_cmds_linux: vec![
    ...
    fn register_alm(&mut self) {
        self.components.insert(
            ...
            ComponentConfig {
                ...
                windows_packages: vec!["git".to_string()],
                download_url: Some(
                    "https://codeberg.org/forgejo/forgejo/releases/download/v10.0.2/forgejo-10.0.2-windows-amd64.exe".to_string(),
                ),
                binary_name: Some("forgejo.exe".to_string()),
                pre_install_cmds_linux: vec![],
                post_install_cmds_linux: vec![],
                pre_install_cmds_macos: vec![],
                post_install_cmds_macos: vec![],
    ...
    fn register_alm_ci(&mut self) {
        self.components.insert(
            ...
            ComponentConfig {
                ...
                windows_packages: vec!["git".to_string(), "nodejs".to_string()],
                download_url: Some(
                    "https://code.forgejo.org/forgejo/runner/releases/download/v6.3.1/forgejo-runner-6.3.1-windows-amd64.exe".to_string(),
                ),
                binary_name: Some("forgejo-runner.exe".to_string()),
                pre_install_cmds_linux: vec![
    ...
    fn register_dns(&mut self) {
        self.components.insert(
            ...
            ComponentConfig {
                ...
                windows_packages: vec![],
                download_url: Some(
                    "https://github.com/coredns/coredns/releases/download/v1.12.4/coredns_1.12.4_windows_amd64.zip".to_string(),
                ),
                binary_name: Some("coredns.exe".to_string()),
                pre_install_cmds_linux: vec![],
                post_install_cmds_linux: vec![
    ...
    fn register_webmail(&mut self) {
        self.components.insert(
            ...
            ComponentConfig {
                ...
                windows_packages: vec!["php".to_string()],
                download_url: Some(
                ...
                pre_install_cmds_windows: vec![],
                post_install_cmds_windows: vec![
                    "php -S 0.0.0.0:8080 -t {{DATA_PATH}}\\roundcubemail".to_string()
                ],
                env_vars: HashMap::new(),
                data_download_list: Vec::new(),
    ...
    fn register_meeting(&mut self) {
        self.components.insert(
            ...
            ComponentConfig {
                ...
                windows_packages: vec![],
                download_url: Some(
                    "https://github.com/livekit/livekit/releases/download/v1.8.4/livekit_1.8.4_windows_amd64.zip".to_string(),
                ),
                binary_name: Some("livekit-server.exe".to_string()),
                pre_install_cmds_linux: vec![],
                post_install_cmds_linux: vec![],
                pre_install_cmds_macos: vec![],
                post_install_cmds_macos: vec![],
    ...
    fn register_table_editor(&mut self) {
        self.components.insert(
            ...
            ComponentConfig {
                ...
                macos_packages: vec![],
                windows_packages: vec!["curl".to_string()],
                download_url: Some("http://get.nocodb.com/windows-x64".to_string()),
                binary_name: Some("nocodb.exe".to_string()),
                pre_install_cmds_linux: vec![],
                post_install_cmds_linux: vec![],
                pre_install_cmds_macos: vec![],
                post_install_cmds_macos: vec![],
    ...
    fn register_doc_editor(&mut self) {
        self.components.insert(
            ...
            ComponentConfig {
                ...
                macos_packages: vec![],
                windows_packages: vec!["gnupg".to_string()],
                download_url: None,
                binary_name: Some("coolwsd.exe".to_string()),
                pre_install_cmds_linux: vec![],
                post_install_cmds_linux: vec![],
                pre_install_cmds_macos: vec![],
                post_install_cmds_macos: vec![],
    ...
    fn register_devtools(&mut self) {
        self.components.insert(
            ...
            ComponentConfig {
                ...
                windows_packages: vec!["git".to_string(), "curl".to_string()],
                download_url: None,
                binary_name: None,
                pre_install_cmds_linux: vec![],
                post_install_cmds_linux: vec![],
    ...
    fn register_bot(&mut self) {
        self.components.insert(
            ...
            ComponentConfig {
                ...
                windows_packages: vec!["nodejs".to_string()],
                download_url: None,
                binary_name: None,
                pre_install_cmds_linux: vec![
                    "curl -fsSL https://deb.nodesource.com/setup_22.x | bash -".to_string(),
    ...
    fn register_system(&mut self) {
        self.components.insert(
            ...
            ComponentConfig {
                ...
                macos_packages: vec![],
                windows_packages: vec!["curl".to_string(), "7zip".to_string(), "git".to_string()],
                download_url: None,
                binary_name: None,
                pre_install_cmds_linux: vec![],
                post_install_cmds_linux: vec![],
